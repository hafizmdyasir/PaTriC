...
void readFromDump(string dumpName, unordered_map<string, string>& dataDump)
{
    ...
    {
        if(line.find('#') == 0 || line.find(':') == -1) continue;
...
bool parseFile(string fullpath)
{
    ...
    string command = "python3 pyparser.py -f " + fullpath;
    int configRead = system(command.c_str());
    if (configRead != 0)
    {
        cerr << endl << "The program encountered the above mentioned error and will now terminate... " << endl;
        return false;
    }
